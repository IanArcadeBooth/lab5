...
void setLEDs(unsigned char);
 ...
 /*
 * Description: Controls the marquee effect of the LEDS using user keyboard input
 * @param a: none (void)
...
int main(void) {
  ...
  while (WHLECONT) { //continues while loop until q is pressed as while loop will always equal one

    
    if (kbhit()) { // Check keyboard without blocking program from running
      key = getchar();

      if (key == 'Q' || key == 'q') { //this if else uses key variable to see which mode the program is in
        break;
      } else if (key == 'L' || key == 'l') {
        direction = 'L';
      } else if (key == 'R' || key == 'r') {
    ...
    if (direction == 'L') { //this if else bit shifts the LED one to the right or left depending on mode
      leds <<= 1;
      if (leds > 0x04) { // if it goes past 3 in either direction it will reset the marquee loop
        leds = 0x01;
      }
    } else {
      leds >>= 1;
...
/*
* Description: Delays program for specified amount of milliseconds using a monotonic clock.
...
* @param dir: 1 to enable retreiving keyboard inputs without disturbing program, 0 for regular operation
* @return: none
* side effects: modifies terminal settings, if the program exits without returning them to normal it could be bad
*/
...
*/
int kbhit(void) {
  struct timeval tv = {0L, 0L}; //sets timeout for checking if button pressed to zero, does not wait
  fd_set fds;

  FD_ZERO(&fds); //clears the temporary file that holds button press
  FD_SET(STDIN_FILENO, &fds); //monitor standard input
  return select(STDIN_FILENO + 1, &fds, NULL, NULL, &tv); //checks if an input is ready
}
